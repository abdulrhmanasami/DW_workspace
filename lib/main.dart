...
import 'package:design_system_shims/design_system_shims.dart' show DWTheme;
...
/// Root application widget with Onboarding Gate.
///
/// Ticket #34: Session-level onboarding gate.
/// Shows onboarding flow on first app launch per session.
...
class _RootAppState extends ConsumerState<RootApp> {
  bool _hasPushedOnboarding = false;

  @override
  void didChangeDependencies() {
    super.didChangeDependencies();

    // Ticket #34: Run onboarding gate once per session.
    if (!_hasPushedOnboarding) {
  ...
  void _maybeShowOnboarding() {
    ...
    if (onboardingState.hasCompletedOnboarding) {
      _hasPushedOnboarding = true;
      return;
    }

    _hasPushedOnboarding = true;

    // Wait for first frame so navigator is ready
    WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialApp(
      ...
      // DWTheme: Unified theme based on Design Tokens (Track A - Ticket #1)
      theme: DWTheme.light(),
      initialRoute: '/',
