/// System Notifications UX Tests
/// Created by: Cursor B-ux
/// Purpose: Unit tests for system notifications UX providers
...
void main() {
  group('systemNotificationsUxProvider', () {
    test('returns only system messages sorted by timestamp', () async {
      ...
      final systemItems =
          container.read(systemNotificationsUxProvider).requireValue;
    ...
    test('returns empty list when feature is unavailable (Sale-Only)',
      ...
      final systemItems = container.read(systemNotificationsUxProvider);

      expect(systemItems.requireValue, isEmpty);
    });
  });
  ...
  group('systemNotificationsEmptyStateProvider', () {
    test('reports empty when no system messages exist', () async {
      ...
      await container.read(notificationsInboxUxProvider.future);
      final emptyState =
          container.read(systemNotificationsEmptyStateProvider);

      expect(emptyState.isEmpty, isTrue);
      expect(emptyState.isFeatureAvailable, isTrue);
      expect(emptyState.titleKey, 'notifications_system_empty_title');
    });

    test('reports unavailable when feature is disabled (Sale-Only)',
        () async {
      final container = ProviderContainer(
        overrides: [
          notificationsAvailabilityProvider.overrideWith(
  ...
  group('systemNotificationDetailUxProvider', () {
    test('resolves system message details by messageId', () async {
      final systemItem = buildNotificationViewModel(
        ...
        action: const NotificationActionOpenSystemMessage(
            messageId: 'policy-update'),
      );
      ...
      final detail = await container
          .read(systemNotificationDetailUxProvider('policy-update').future);
    ...
    test('throws when feature is unavailable (Sale-Only)', () async {
      final container = ProviderContainer(
        overrides: [
          notificationsAvailabilityProvider.overrideWith(
            (ref) => NotificationsAvailabilityNotifier()..markDisabled(),
      ...
      await expectLater(
        container.read(systemNotificationDetailUxProvider('any').future),
        throwsStateError,
      );
    });
  });
  ...
  group('SystemNotificationsUxController', () {
    test('marks system message as read by id', () async {
      ...
      final controller =
          container.read(systemNotificationsUxControllerProvider);
