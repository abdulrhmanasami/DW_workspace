/// Promotions Notifications UX Tests
/// Created by: Cursor B-ux
/// Purpose: Unit tests for promotions notifications UX providers
...
void main() {
  group('promotionsNotificationsUxProvider', () {
    test('exposes only promotion items sorted by timestamp', () async {
      ...
      final systemItem = buildNotificationViewModel(
        ...
        action:
            const NotificationActionOpenSystemMessage(messageId: 'sys-1'),
      ...
      final promotions =
          container.read(promotionsNotificationsUxProvider).requireValue;
      ...
      expect(
          promotions
              .every((item) => item.type == NotificationType.promotion),
          isTrue);
    });

    test('returns empty list when feature is unavailable (Sale-Only)',
        () async {
      final container = ProviderContainer(
        overrides: [
          notificationsAvailabilityProvider.overrideWith(
      ...
      final promotions = container.read(promotionsNotificationsUxProvider);

      expect(promotions.requireValue, isEmpty);
    });
  });
  ...
  group('promotionsNotificationsEmptyStateProvider', () {
    test('returns empty state when no promotions are available', () async {
            ...
            (ref) => Stream<List<NotificationListItemViewModel>>.value(
              [
                buildNotificationViewModel(
                  ...
                  action:
                      const NotificationActionOpenOrderDetails('order-1'),
                ),
              ],
            ),
          ),
      ...
      await container.read(notificationsInboxUxProvider.future);
      final emptyState =
          container.read(promotionsNotificationsEmptyStateProvider);

      expect(emptyState.isEmpty, isTrue);
      expect(emptyState.isFeatureAvailable, isTrue);
      expect(emptyState.titleKey, 'notifications_promotions_empty_title');
      expect(
          emptyState.primaryAction, isA<NotificationActionOpenPromotions>());
    });

    test('returns unavailable state when feature is disabled (Sale-Only)',
        () async {
      final container = ProviderContainer(
        overrides: [
          notificationsAvailabilityProvider.overrideWith(
          ...
          notificationsInboxUxProvider.overrideWith(
            (ref) => Stream<List<NotificationListItemViewModel>>.value(
              [
                ...
                buildNotificationViewModel(id: 'promo-2')
              ],
            ),
          ),
        ],
  ...
  group('PromotionsNotificationsUxController', () {
    test('marks only promotion notifications as read when requested',
        () async {
      final repository = InMemoryNotificationsInboxRepository(
        initialEntries: [
          buildNotificationEntry(
              id: 'promo-unread', type: NotificationType.promotion),
          buildNotificationEntry(
              id: 'system-1', type: NotificationType.system),
        ],
      );
      final container = ProviderContainer(
        overrides: [
      ...
      final controller =
          container.read(promotionsNotificationsUxControllerProvider);
    ...
    test('clears only promotion notifications', () async {
      final repository = InMemoryNotificationsInboxRepository(
        initialEntries: [
          buildNotificationEntry(
              id: 'promo-1', type: NotificationType.promotion),
          buildNotificationEntry(
              id: 'promo-2', type: NotificationType.promotion),
          buildNotificationEntry(
              id: 'system-2', type: NotificationType.system),
        ],
      );
      final container = ProviderContainer(
        overrides: [
      ...
      expect(repository.deletedNotifications,
          containsAll(['promo-1', 'promo-2']));
